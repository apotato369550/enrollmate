...
class PDFExporter {
  ...
  static _buildTimetable(courses) {
    ...
    // Generate 30-min slots from 9 AM to 5:30 PM
    const slots = [];
    for (let hour = 9; hour <= 17; hour++) {
      slots.push({ hour, minute: 0 });
      if (hour < 17) slots.push({ hour, minute: 30 });
    }
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
        <thead>
          <tr style="background-color: #fff; border: 1px solid #000;">
            <th style="border: 1px solid #000; padding: 8px; width: 120px; font-weight: bold;">Time</th>
    `;

    days.forEach(day => {
      html += `<th style="border: 1px solid #000; padding: 8px; font-weight: bold;">${day}</th>`;
    });
    html += `</tr></thead><tbody>`;

    // Add rows
    slots.forEach(slot => {
      const timeStr = this._formatTime(slot.hour, slot.minute);
      const slotKey = `${slot.hour}:${slot.minute}`;

      html += `<tr><td style="border: 1px solid #000; padding: 6px; background-color: #f9f9f9; font-weight: bold;">${timeStr}</td>`;

      days.forEach(day => {
        const cell = grid[day] && grid[day][slotKey];
        if (cell) {
          if (cell.isFirst) {
            html += `<td style="border: 1px solid #000; padding: 6px; background-color: ${cell.color}; text-align: center; font-weight: bold;">${cell.code}</td>`;
          } else {
            html += `<td style="border: 1px solid #000; padding: 6px; background-color: ${cell.color};"></td>`;
          }
        } else {
          html += `<td style="border: 1px solid #000; padding: 6px; background-color: #fff;"></td>`;
        }
      });
