...
class Section {
  ...
  isViable(constraints) {
    ...
    const earliestMinutes = this._toMinutes(constraints.earliestStart);
    const latestMinutes = this._toMinutes(constraints.latestEnd);
...
class ScheduleGenerator {
   ...
   */
  #backtrack(index, current, results, viableSections) {
    ...
    for (const section of viableSections[index]) {
      ...
      if (!hasConflict) {
        ...
        this.#backtrack(index + 1, current, results, viableSections);
  ...
  #createScheduleObject(sections) {
    ...
    const endsByPreferred = latestEnd <= this._toMinutes(this.constraints.latestEnd);
