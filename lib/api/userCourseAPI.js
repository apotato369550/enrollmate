  ...
  static async saveCourse(userId, courseData, source = 'manual') {
    try {
      ...
      const { data: existing, error: existingError } = await supabase
        ...
        .select('id')
        .eq('user_id', userId)
        .eq('course_code', courseData.courseCode)
        .eq('section_group', courseData.sectionGroup)
        .maybeSingle();
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          course_name: courseData.courseName || courseData.course_name,
          section_group: courseData.sectionGroup || courseData.section_group,
          schedule: courseData.schedule,
          enrolled_current: courseData.enrolledCurrent || courseData.enrolled_current || 0,
          enrolled_total: courseData.enrolledTotal || courseData.enrolled_total,
